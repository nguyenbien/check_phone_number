...
public class SwiftCheckPhoneNumberPlugin: NSObject, FlutterPlugin {
  public static func register(with registrar: FlutterPluginRegistrar) {
    let channel = FlutterMethodChannel(name: "com.biennt/check_phone_number", binaryMessenger: registrar.messenger())
    let instance = SwiftCheckPhoneNumberPlugin()
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        private func validate(_ call: FlutterMethodCall, result: FlutterResult){
            guard
                ...
                let region = arguments["phone"] as? String
                else {
        ...
        private func parse(_ call: FlutterMethodCall, result: FlutterResult) {
            guard
                let arguments = call.arguments as? [String : Any],
                let string = arguments["phone"] as? String
                else {
                    result(FlutterError(code: "InvalidArgument",
                                        message: "The 'string' argument is missing.",
                                        details: nil))
        ...
        private func parseList(_ call: FlutterMethodCall, result: FlutterResult) {
            guard
                let arguments = call.arguments as? [String : Any],
                let strings = arguments["phones"] as? [String]
                else {
                    result(FlutterError(code: "InvalidArgument",
